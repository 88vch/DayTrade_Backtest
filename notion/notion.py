from backtest import info
from backtest import setups
import requests
...
headers = {
    "Authorization": "Bearer " + info.NOTION_TOKEN,
...
def get_pages(num_pages: int):
    try:
        url = f"https://api.notion.com/v1/databases/{info.DATABASE_ID}/query"
        # payload = {"parent": {"database_id": info.DATABASE_ID}, "properties": data}
...
- a setup is valid if it exists amongst our setups.SETUPS
"""
def is_valid_setup(setup: str) -> bool:
    if setup in setups.SETUPS:
        return True
    return False
